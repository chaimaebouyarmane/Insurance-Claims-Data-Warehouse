...
package projet11.dim_claim_event_0_1;
...
/**
 * Job: dim_claim_event Purpose: <br>
 * Description: <br>
 * 
 ...
 * @status
 */
public class dim_claim_event implements TalendJob {
	...
	private final String jobName = "dim_claim_event";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					dim_claim_event.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(dim_claim_event.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class dim_statusStruct implements routines.system.IPersistableRow<dim_statusStruct> {
		final static byte[] commonByteArrayLock_PROJET11_dim_claim_event = new byte[0];
		static byte[] commonByteArray_PROJET11_dim_claim_event = new byte[0];

		public Integer stat_key;

		public Integer getStat_key() {
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_PROJET11_dim_claim_event.length) {
					if (length < 1024 && commonByteArray_PROJET11_dim_claim_event.length == 0) {
						commonByteArray_PROJET11_dim_claim_event = new byte[1024];
					} else {
						commonByteArray_PROJET11_dim_claim_event = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_PROJET11_dim_claim_event, 0, length);
				strReturn = new String(commonByteArray_PROJET11_dim_claim_event, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_PROJET11_dim_claim_event, 0, length);
				strReturn = new String(commonByteArray_PROJET11_dim_claim_event, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_PROJET11_dim_claim_event) {

				try {

					int length = 0;

					this.stat_key = readInteger(dis);
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				writeInteger(this.stat_key, dos);
		...
		public String toString() {
			...
			sb.append("[");
			sb.append("stat_key=" + String.valueOf(stat_key));
	...
	public void tDBInput_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				if (execStat) {
					runStat.updateStatOnConnection(resourceMap, iterateId, 0, 0, "dim_status");
				}
				...
				final String decryptedPassword_tDBOutput_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:vNo4rwzVlm/7cyRrRBksDKVlagq86tjic47QNhnp");
				...
				if (dbschema_tDBOutput_1 == null || dbschema_tDBOutput_1.trim().length() == 0) {
					tableName_tDBOutput_1 = ("dim_status");
				} else {
					tableName_tDBOutput_1 = dbschema_tDBOutput_1 + "." + ("dim_status");
				}
				String tableNameForSearch_tDBOutput_1 = "" + ((String) "dim_status") + "";
				...
				try (java.sql.Statement stmtCreate_tDBOutput_1 = conn_tDBOutput_1.createStatement()) {
					stmtCreate_tDBOutput_1.execute("CREATE TABLE " + tableName_tDBOutput_1
							+ "(stat_key INT ,CURRENT_STATUS VARCHAR2(50)  ,CURRENT_SUBSTATUS VARCHAR2(50)  )");
				}
				String insert_tDBOutput_1 = "INSERT INTO " + tableName_tDBOutput_1
						+ " (stat_key,CURRENT_STATUS,CURRENT_SUBSTATUS) VALUES (?,?,?)";
...
// ###############################
				...
				class KeyStruct_tUniqRow_1 {
					...
					public int hashCode() {
						if (this.hashCodeDirty) {
							...
							result = prime * result
									+ ((this.CURRENT_STATUS == null) ? 0 : this.CURRENT_STATUS.hashCode());
					...
					public boolean equals(Object obj) {
						...
						} else if (!this.CURRENT_STATUS.equals(other.CURRENT_STATUS))
				...
				final String decryptedPassword_tDBInput_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:JUrLEcv7WsoJcGi7AuitAm0xQi2635t1JSPjA7Zd");
				...
				try {
					...
					while (rs_tDBInput_1.next()) {
						...
						row2 = null;
						if (row1.CURRENT_STATUS == null) {
							finder_tUniqRow_1.CURRENT_STATUS = null;
						} else {
							finder_tUniqRow_1.CURRENT_STATUS = row1.CURRENT_STATUS.toLowerCase();
						}
						...
						if (!keystUniqRow_1.contains(finder_tUniqRow_1)) {
							...
							if (row1.CURRENT_STATUS == null) {
								new_tUniqRow_1.CURRENT_STATUS = null;
							} else {
								new_tUniqRow_1.CURRENT_STATUS = row1.CURRENT_STATUS.toLowerCase();
							}
...
// # Output table : 'dim_status'
								dim_status_tmp.stat_key = Numeric.sequence("s1", 1, 1);
								dim_status_tmp.CURRENT_STATUS = row2.CURRENT_STATUS;
								dim_status_tmp.CURRENT_SUBSTATUS = row2.CURRENT_SUBSTATUS;
...
// Start of branch "dim_status"
							if (dim_status != null) {
								...
								if (dim_status.stat_key == null) {
									pstmt_tDBOutput_1.setNull(1, java.sql.Types.INTEGER);
								} else {
									pstmt_tDBOutput_1.setInt(1, dim_status.stat_key);
								}

								if (dim_status.CURRENT_STATUS == null) {
									pstmt_tDBOutput_1.setNull(2, java.sql.Types.VARCHAR);
								} else {
									pstmt_tDBOutput_1.setString(2, dim_status.CURRENT_STATUS);
								}

								if (dim_status.CURRENT_SUBSTATUS == null) {
									pstmt_tDBOutput_1.setNull(3, java.sql.Types.VARCHAR);
								} else {
									pstmt_tDBOutput_1.setString(3, dim_status.CURRENT_SUBSTATUS);
								}
...
// ###############################      
				...
				if (execStat) {
					runStat.updateStat(resourceMap, iterateId, 2, 0, "dim_status");
				}
	...
	public static void main(String[] args) {
		final dim_claim_event dim_claim_eventClass = new dim_claim_event();

		int exitCode = dim_claim_eventClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = dim_claim_event.class.getClassLoader()
					.getResourceAsStream("projet11/dim_claim_event_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = dim_claim_event.class.getClassLoader()
		...
		if (false) {
			System.out.println(
					(endUsedMemory - startUsedMemory) + " bytes memory increase when running : dim_claim_event");
		}
...
/************************************************************************************************
 * 92959 characters generated by Talend Open Studio for Data Integration on the
 * 5 janvier 2023 à 05:49:29 WEST
 ************************************************************************************************/